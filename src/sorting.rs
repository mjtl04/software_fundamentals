...
fn get_votes(candidates: &mut [Candidates; 5]) {
	loop {
		println!("Enter the Cadidate's Number you wish to vote for: ({EXIT_KEY} to exit) ");
		let vote_input = get_input();
		if vote_input == EXIT_KEY {
			break;
		} else if vote_input < EXIT_KEY {
